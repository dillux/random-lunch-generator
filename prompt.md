...
## Q (fixed prompt): Now write the code in a single `index.html` (HTML + CSS + JS) with a random lunch picker.

Requirements for the icons:
- Use **Font Awesome 6.4.0 Free** from `https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css`.
- Use **only icons that exist in the Free Solid set of version 6.4.0** (check the name at fontawesome.com/icons with the "Free" filter). Do NOT invent icon names (e.g. `fa-pasta`, `fa-bowl`) and do NOT use Pro-only icons (e.g. `fa-bowl-hot`).
- Store each dish as `{ name, icon }` and list next to it why this icon was chosen.
- Add a fallback: after rendering, if the icon's `::before` content is `none`, replace it with `fas fa-utensils`, so the picture is never empty.
- Before finishing, list every icon class you used and confirm each one exists in Font Awesome 6.4.0 Free.
